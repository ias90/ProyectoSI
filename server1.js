...
models.sequelize.sync().then(function() {
    ...
    server.register([Inert, Basic], (err) => {
            ...
            config: {
                handler: function (request, reply) {
                    ...
                    Bcrypt.hash(request.payload.password, saltRounds, function(err, hash) {
                        ...
                        // asignando contraseñaa hasheada
                        request.payload.password = hash;
                        // crear usuario (Note que se usa el modelo Users puede ser cualquier otro que cumpla)
                        estudiantes.create(request.payload).then(
                            (result)=>{
                                if(result) {
        ...
        server.route({
            ...
            handler: function (request, reply) 
            {
                reply.file('./vistas/index.html').code(401);
            }
        }),
...
const validate = function (request, correo, password, callback) {
    // console.log(request.path) //'/nuevoUsuario'
    // este metodo 
    estudiantes.findOne({
        where: {
            correo: correo
            ...
            if(!result) {
                ...
                Bcrypt.compare(password, result.password, (err, isValid) => {
                    return callback(err, isValid, { id: result.id, nombre: result.nombre });
                });
            }
        });
};

//// Ruta que requiere de autenticación
/*server.route({
            method: 'GET',
            path: '/',
            config: {
